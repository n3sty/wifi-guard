// WiFi Guard Extension - Background Service Worker (TypeScript)
// Handles background tasks and extension lifecycle

import { SecurityCheck, ScanResult } from './types';
...
async function handleSecurityScan(sendResponse: (response?: any) => void) {
  try {
...
async function performActualChecks(tab: chrome.tabs.Tab, checks: SecurityCheck[]) {
  try {
    ...
    const overallStatus: 'safe' | 'caution' | 'danger' = hasFailures ? 'danger' : hasWarnings ? 'caution' : 'safe';
    ...
    try {
      ...
      console.log('Could not notify popup (likely closed):', error instanceof Error ? error.message : 'Unknown error');
    }
...
// Get scan history
async function getScanHistory(sendResponse: (response?: any) => void) {
  try {
    const { scanHistory = [] } = await chrome.storage.local.get(['scanHistory']);
...
// Clear scan history
async function clearScanHistory(sendResponse: (response?: any) => void) {
  try {
